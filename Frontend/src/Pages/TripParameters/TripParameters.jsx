...
import { addTripApi, getAllVehicles, getConductorsListApi, getDriversListApi, getVechileByIdVD, updateVehicleStatus } from '../../services/allAPI';
...
// 
  ...
  const getAllDriversList = async () => {
    try {
      ...
      if (result.status == 200) {
        const filteredDrivers = result.data.filter(driver => driver.on_leave === "Available")
  ...
  const getAllConductorsList = async () => {
    try {
      ...
      if (result.status == 200) {
        const filteredConductors = result.data.filter(conductor => conductor.on_leave === "Available")
  ...
  const handleSchedule = async () => {
    if (!outboundTrip.vehicle_id || !outboundTrip.driver_id || !outboundTrip.conductor_id || !outboundTrip.start_date || !outboundTrip.end_date || !outboundTrip.departure_location.depo || !outboundTrip.arrival_location.depo || !outboundTrip.start_time || !outboundTrip.end_time) {
      alert("Fill All Fields")
    } else if (!returnTrip.vehicle_id || !returnTrip.driver_id || !returnTrip.conductor_id || !returnTrip.start_date || !returnTrip.end_date || !returnTrip.departure_location.depo || !returnTrip.arrival_location.depo || !returnTrip.start_time || !returnTrip.end_time) {
      alert("Fill All Fields")
    } else if (
                ...
                <Card.Body>
                  ...
                  <Form>
                    ...
                    <Form.Group className="mt-4">
                      ...
                      <Row className="mt-3">
                        <Col>
                          <Form.Label className="mb-1" style={{ fontSize: "14px" }}>Start Depo</Form.Label>
                          <div className='postion-relative  w-100 p-1'>
                            ...
                            <ul className="position-absolute" style={{ width: "45%" }}>
                              {
                                depoList
                                  .filter(item => !startDepo ? false : item.toLowerCase().search(startDepo.toLowerCase()) === -1 ? false : true)
                                  .slice(0, 5)
                                  .map((item, index) => (
                                    <li
                                      key={index}
                        ...
                        <Col>
                          <Form.Label className="mb-1" style={{ fontSize: "14px" }}>End Depo</Form.Label>
                          <input
                            type="text"
                            className="form-control"
                          ...
                          <ul className="position-absolute" style={{ width: "45%" }}>
                            {
                              depoList
                                .filter(item => !endDepo ? false : item.toLowerCase().search(endDepo.toLowerCase()) === -1 ? false : true) // Filter based on endDepo
                                .slice(0, 5)
                                .map((item, index) => (
                                  <li
                                    key={index}
                      ...
                      <Row className="mt-3">
                        <Col>
                          ...
                          <ul className="position-absolute" style={{ width: "45%" }}>
                            {
                              depoList
                                .filter(item => !returnStartDepo ? false : item.toLowerCase().includes(returnStartDepo.toLowerCase()))
                                .slice(0, 5)
                                .map((item, index) => (
                                  <li
                                    key={index}
                        ...
                        <Col>
                          ...
                          <ul className="position-absolute" style={{ width: "45%" }}>
                            {
                              depoList
                                .filter(item => !returnEndDepo ? false : item.toLowerCase().includes(returnEndDepo.toLowerCase()))
                                .slice(0, 5)
                                .map((item, index) => (
                                  <li
                                    key={index}
