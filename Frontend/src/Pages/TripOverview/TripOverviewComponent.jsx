  ...
  const getTripInitialData = async () => {
    try {
      ...
      const res =
        user.role === "Admin"
          ? await getAllTripApi("")
          : await getTripOfDepotApi(user.depoName, "");
      setTrips(res.data);
    } catch (error) {
                ...
                {depoList
                  ...
                  .map((item, index) => (
                    <li
                      ...
                      onClick={() => setDepo(item.code + " " + item.name)}
                    >
                      {item.code} {item.name}
                    </li>
                  ))}
          ...
          <div className="row">
            <div className="col">
              ...
              <span className="fw-semibold">
                {filteredTrips[chosenTrip]?.waybill_Number}
              </span>
            </div>
            <div className="col">
              <p className="text-secondary m-0">
              ...
              <span className="fw-semibold">
                {filteredTrips[chosenTrip]?.start_time}
              </span>
            </div>
            <div className="col">
              ...
              <span className="fw-semibold">
                {filteredTrips[chosenTrip]?.end_time}
              </span>
            </div>
          </div>
        </div>
      ...
      <Table striped bordered hover>
        ...
        <tbody>
          {filteredTrips.map((item, index) => (
            <tr
              key={index}
              style={index === chosenTrip ? { backgroundColor: "#f0f0f0" } : {}}
              onClick={() => setChosenTrip(index)}
            >
              <td>
                <div className="d-flex align-items-center">
                  <div
                    className={`circle p-1 me-2 ${
                      index === chosenTrip ? "circlePick" : ""
                    }`}
                  >
              ...
              <td
                className={`${tripStatusColor(item?.status)} ${
                  index === chosenTrip ? "text-primary" : ""
                }`}
              >
              ...
              <td>
                <div>
                  ...
                  <span className="fw-semibold">
                    ...
                    {
                      depoList.find(
                        (d) => d.code === item?.departure_location.depo
                      )?.name
                    }
                  </span>
                </div>
              ...
              <td>
                <div>
                  ...
                  <span className="fw-semibold">
                    ...
                    {
                      depoList.find(
                        (d) => d.code === item?.arrival_location.depo
                      )?.name
                    }
                  </span>
                </div>
              ...
              <td>
                {drivers.find((item2) => item2._id == item?.driver_id)
                  ?.EmployeeName || "NA"}
              </td>
              <td>
                {Conductors.find((item2) => item2._id == item?.conductor_id)
                  ?.EmployeeName || "NA"}
              </td>
            </tr>
          ))}
